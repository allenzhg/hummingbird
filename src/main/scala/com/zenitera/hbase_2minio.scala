...
import utils.toolsUtils.{combineData, saveToMinio}
...
object hbase_2minio {
  def main(args: Array[String]): Unit = {
    ...
    hbase_2minio.handleData(conf, jsonObject)
  ...
  def handleData(conf: Configuration, jsonObject: JSONObject): Unit = {
    ...
    val endpoint = getParameter("endpoint")
    val bucket_name = getParameter("bucket_name")
    val minio_user = getParameter("minio_user")
    val minio_pass = getParameter("minio_pass")
    val minio_dir = getParameter("minio_dir")
    val minioClient: MinioClient =
      MinioClient.builder()
        .endpoint(s"${endpoint}")
        .credentials(s"${minio_user}", s"${minio_pass}")
        .build()
    ...
    try {
      ...
      while (rowIterator.hasNext) {
          ...
          if (compress_type == "snappy") {
            for (key <- sortedStrings) {
              ...
              if (compress_field.contains(value.toString)) {
                var data_fix = Bytes.toBytes("null")
                try {
                  if (data.isEmpty) {
                    data_fix = Bytes.toBytes("null")
                    jsonContent.put(value.toString, Bytes.toString(data_fix))
                  } else {
                    data_fix = Snappy.uncompress(data)
                    jsonContent.put(value.toString, Bytes.toString(data_fix).replaceAll("[\u0000-\u000F]", "").replaceAll("\uFFFD", ""))
                  }
                } catch {
                  case e: Exception => Files.write(error_rowkeys_path, combineData(error_rowkeys_path_str, id).getBytes(StandardCharsets.UTF_8))
        ...
        saveToMinio(id, jsonContent.toJSONString, minioClient, bucket_name, minio_dir)
        jsonContent.clear()
      }
    } catch {
